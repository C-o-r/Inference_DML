...
for (i in 43:200){
  ...
  r <- foreach(k = 1:split, .combine='rbind', .inorder=FALSE, .packages=c('MASS','randomForest','neuralnet','gbm', 'sandwich', 'hdm', 'nnet', 'rpart','glmnet')) %dopar% { 
    ...
    dml <- DoubleML(data=data, y=y, d=d, z=NULL, xx=x, xL=xl, methods=methods, DML="DML2", nfold=2, est="plinear", arguments=arguments, ensemble=ensemble, silent=TRUE, trim=c(0.01,0.99)) 
...
par( mfrow = c( 1, 1 ) )
clplot(x,y ,
       ylab="time of Trees",xlab	="amount of observations", main="", lwd=5, levels=c(1,2,3,4,5), col=my_colors, showcuts=T , bty="n")

write.csv(y, file = "c1_TreesO_43_200.csv", row.names = FALSE)
